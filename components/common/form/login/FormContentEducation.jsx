...
const FormContentEducation = (props) => {
  ...
  const handleSubmitDetails = async () => {
    if(degree && school && fromDate && toDate && description){
      try {
        if (id > 0) {
          await supabase
  ...
  return (
    <div className="form-inner">
      <h3>{props.id > 0 ? 'Edit Education Details' : 'Add Education Details'}</h3>
      <form method="post">
        ...
        <div className="form-group">
          <label>{props.sub_title} <span className="required">(required)</span></label>
          <input
            type="text"
            name="school-name"
...
export default FormContentEducation;
